...
const TABLE: Array[Dictionary] = [
	...
	},
	{
		...
		"waves": [
			...
			#
			# ── 여기에 적 악사를 넣는다 ──────────────────────────────────
			# [지원 차단](회복하는 적을 친다)이 5판 통틀어 8회밖에 안 걸렸다.
			# 적 악사가 3·5 스테이지에만 있었기 때문이다. 표적 축의 대응
			# 모듈을 처음 만나는 자리가 3스테이지(승률 33%)면 배우기 전에
			# 진다.
			#
			# 2스테이지는 아직 이길 수 있는 판이라 배우기 좋다. 그리고 이
			# 페이즈의 문제("붙는 길이 막혀 있다")와도 맞물린다 - 포탑을
			# 때리는 동안 악사가 그걸 되돌리므로, 갉는 것으로는 안 끝난다.
			# 뒤를 끊거나 회복원을 먼저 지워야 한다는 것이 여기서 처음 나온다.
			[
	...
	},
	{
		...
		"waves": [
			...
			# ── 악사를 뺐다 ──────────────────────────────────────────────
			# 신호기가 표적을 흐리는 동안 악사가 뒤에서 살리는 구성이었다.
			# 둘이 겹치니 "후열을 끊는다" 가 아니라 **끊을 수가 없다** 가 됐다 -
			# 신호기 때문에 후열로 못 가고, 못 가는 동안 회복이 계속 들어온다.
			# 실측 3단계 승률 17%%, 그중 정체패가 32%% 였다.
			#
			# 방벽은 신호기와 방패병으로 충분하다. 회복까지 얹는 것은 벽을
			# 두껍게 하는 게 아니라 **문을 잠그는** 쪽이었다.
			[
