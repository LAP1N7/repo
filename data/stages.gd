		...
		"waves": [
			...
			[
				...
				{ "type": "turret", "pos": Vector2i(6, 3), "cards": [],
				  "traits": ["immobile"] },
				# ── 악사를 뺐다 ──────────────────────────────────────────
				# 고정포탑 둘이 이미 앞을 잠그고 있는데 그 뒤에 회복까지 서면,
				# 2단계가 "회복원을 먼저 지워라" 를 **가르치는** 판이 아니라
				# 그것을 못 하면 못 이기는 판이 된다. 배우는 자리에서 답을
				# 하나로 좁히면 그건 학습이 아니라 관문이다.
				#
				# 회복원을 끊는 법은 3단계가 방벽과 함께 제대로 가르친다.
			],
		],
	},
	{
