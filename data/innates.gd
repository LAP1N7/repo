...
## 붙으러 가고 원거리는 자리를 지킨다. 그래서 **모듈이 하나도 없는** 궁수는
## 서서 쏘기만 한다 - "모듈을 사야 대원이 자기 뜻대로 움직인다" 는 경제의
## 근거가 여기서 나온다.
##
## ── 표적 모듈을 꽂으면 이 규칙은 풀린다 ──────────────────────────────────
## 표적 카드는 전부 "쫓는다" 라고 적혀 있다. 그런데 예전에는 원거리가 표적을
## 정해 놓고도 안 움직여서, 카드가 자기 문장과 모순이었다. 지금은 표적을
## 지정하면 쫓는다 - 다만 **사거리 안까지만** 간다(core/rules.gd 참조).
## 궁수는 3칸에서 선다. 근접으로 걸어 들어가지 않는다.
