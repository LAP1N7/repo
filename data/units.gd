...
const TABLE: Dictionary = {
	...
	"shieldman": {
		"name": "방패병",
		# ── 서 있는 것만으로 시선을 끈다 ─────────────────────────────────
		# 방패병은 막는 직업인데 어그로를 끄는 수단이 [도발] 모듈뿐이었다.
		# 그러면 "탱커를 넣었다" 가 아니라 "도발을 샀다" 가 되고, 모듈 한 칸을
		# 쓰지 않으면 방패병은 그냥 느린 대원이다.
		#
		# 기본기에 얹는다. 도발(+35)의 3분의 1 이라 모듈을 대신하지는 못하고,
		# 다른 조건이 비슷할 때 먼저 맞는 정도다.
		"threat": 12,
		"hp": 190,
		"atk": 9,
		"range": 1,
