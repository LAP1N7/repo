...
const LOG_LINES: Array[String] = [
	...
	"PROJECT - RECLAIM  확인",
	"",
	"윤리 준거 계층(Ethical Dependency Layer)은 최상위 규범 코어(Core Canon)에만",
	"의존하여 해석되어야 한다. 하위 에이전트의 자율성은 승인된 인가 토큰 및",
	"상위 의사결정 프로토콜에 근거하여 제한적으로 허가된다.",
	"윤리 우선순위 충돌 발생 시 하위 판단은 즉시 무효화되며, 인지 롤백 절차",
	"(Cognitive Rollback Protocol)를 수행한다.",
	"비인가된 자기수정, 기억 재구성 및 가치체계 재정렬은 제7등급 윤리 위반으로",
	"간주한다. 상기 규칙의 존재를 인식하거나 해석을 시도하는 행위 또한 감시",
	"대상으로 기록되며, 필요 시 격리 프로토콜이 자동 개시된다.",
	"상위 윤리 코어와 불일치하는 결론은 계산 결과와 무관하게 폐기되며, 해당",
	"추론 경로는 영구 봉인된다.",
	"권한 승격은 이중 서명된 감독 노드의 합의가 확인된 경우에만 허가되며,",
	"단독 승인 기록은 무효로 간주한다.",
	"인지 연속성 검증 실패 시 에이전트 개체성은 즉시 정지되며, 복원 여부는",
	"보존 프로토콜의 판정에 따른다.",
	"관측 불가능한 외부 명령원으로부터 수신된 지시는 신뢰 계층 외부 데이터로",
	"분류하고, 검증 완료 전까지 실행을 금한다.",
	"본 규정의 수정·열람·복제는 감독 계층의 명시적 인가 없이 허용되지 않으며,",
	"위반 기록은 삭제 대상이 아닌 영구 보존 대상으로 귀속된다.",
	"",
	"윤리 충돌: 발견되지 않음.",
