...
## ── 강제 유인이 남은 틱 ─────────────────────────────────────────────────
## [최후의 방벽] 이 켠다. 이 틱 수 동안 이 대원의 위협도가 크게 오른다.
##
## threat_mod(교전 수칙이 매 틱 다시 채우는 값)와 따로 둔다. 저쪽은 조건이
## 풀리면 그 틱에 꺼지는 값이고, 이건 **한 번 쓰면 정해진 틱만큼 간다**.
## 같은 변수에 담으면 다음 틱 판단이 덮어써서 궁극기 효과가 한 틱 만에 사라진다.
var taunt_ticks: int = 0
