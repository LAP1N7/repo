...
## ── 적도 강화되므로 예산도 따라 오른다 ──────────────────────────────────
## 3판부터 적에게 강화 단계를 붙였다(Stages.ENEMY_UPGRADE). 한 단계는 HP·ATK
## 각 +15% 라 곱하면 체감 +30% 가 넘는다 - 실측으로 4판 승률이 25% -> 6% 로
## 무너졌다.
##
## 적이 세지는 것 자체는 의도다. 지휘관이 강화로 세지는데 적이 그대로면 뒤로
## 갈수록 판이 쉬워지고, 그러면 앞에서 짠 편성을 그대로 밀어도 통한다.
##
