...
func _draw() -> void:
	...
	# ── 테두리를 따라 도는 바코드 ────────────────────────────────────────
	# 카드 윗변이 이름 하나로 끝나서 오른쪽이 비어 있었다. 검은 세로선을
	# 폭이 다르게 늘어놓으면 그 자리가 "비어 있다" 가 아니라 **인쇄물의 여백**이
	# 된다. 물자에 붙은 표찰이라는 뜻이고, 이 게임의 다른 판들과도 맞물린다.
	#
	# 폭은 고정 배열에서 꺼낸다. 난수로 흔들면 카드마다 달라져서 무늬가 아니라
	# 잡음이 된다 - 같은 표찰이 같은 모양이어야 표찰로 읽힌다.
	var bar := Color(0, 0, 0, 0.55)
	const WID: Array[float] = [2.0, 1.0, 1.0, 3.0, 1.0, 2.0, 1.0, 1.0, 2.0, 1.0]
	var bx := s.x - 10.0
	for i in WID.size():
		var w2: float = WID[i]
		bx -= w2
		if bx < s.x * 0.42:
			break
		draw_rect(Rect2(bx, 7.0, w2, 11.0), bar)
		bx -= 2.0
	# 아래 변에도 짧게 한 줄. 위아래가 같은 어법이면 테두리 전체가 표찰이 된다.
	var by := s.y - 7.0
	var bx2 := 10.0
	for i in WID.size():
		var w3: float = WID[(i + 3) % WID.size()]
		if bx2 + w3 > s.x * 0.46:
			break
		draw_rect(Rect2(bx2, by - 7.0, w3, 7.0), Color(0, 0, 0, 0.42))
		bx2 += w3 + 2.0
