...
##
## 화면마다 비어 있는 구역이 다르다. 전투 화면은 판(x116~740, y128~512)과
## 오른쪽 열(x892~1248 - 전황판·교전 기록·조작 버튼)이 꽉 차 있어서, 자동
## 계산에 맡기면 매번 판 한가운데를 덮는다. 앵커가 없는 대사는 대본이 자리를
## 직접 고른다.
const PLACES := {
...
## 아래쪽 자리의 바닥선.
##
## 전투 화면 하단은 대원 바(x48~836 · y566~662)가 가져갔다. 예전 값 584 는
## 조작줄이 y592 부터였을 때 잡은 것이라, 바가 위로 올라온 지금은 말풍선 아랫단이
## 대원 카드 머리를 26px 잘라먹는다. 바로 그 대원 바를 설명하는 대사(battle_t5)가
## 있으므로 여기를 덮으면 안 된다.
##
## 위로 올린 만큼 말풍선은 판의 아래 두 줄(y384~512)에 걸치는데, 훈련장은
## 그 줄이 비어 있어서 가릴 것이 없다.
const BOTTOM_Y := 558.0
