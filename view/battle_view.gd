...
## 한눈에 안 들어왔다.
##
## 반대로 가야 한다. 칸은 작게 두고 대원이 그 칸을 **밟고 서서** 위로
## 넘치게 한다. 그러면 시선이 칸이 아니라 사람에게 간다.
##
...
##
## "누가 실제로 일했는가" 를 한눈에 보여 준다. 이 게임은 조작이
...
func _process(_delta: float) -> void:
	# ── 판 위에서 마우스가 올라간 대원 ───────────────────────────────────
	# 판 위의 개체에 마우스를 올리면 그 자리에서 정체를 밝힌다.
...
func _zones_body(c: CanvasItem, outline_only: bool = false) -> void:
	...
	for u in battle.units:
		...
		if not outline_only:
			...
			#
			# 그래서 채우는 대신 **선으로 짠 상자**로 간다. 아주 옅은
...
func _build_ui() -> void:
	...
	# ── 하단 대원 바 ────────────────────────────────────────────────────
	# 하단 정보 바의 어법이다. 오른쪽에 글자로 늘어놓던 출전 정보를 얼굴
...
##
## ── 궁극기 연출을 이루는 네 겹 ──────────────────────────────────────
## 이 연출이 "특별한 한 방" 으로 읽히려면 네 가지가 같이 와야 한다.
