...
func _draw() -> void:
	...
	# 앞줄 대원의 몸이 뒷줄의 발밑을 덮고, 게다가 링은 그림자·범위 표시와
	# 같은 자리에 있어 색이 섞였다. 실제로 판이 붐비면 아군과 적이 안 갈렸다.
	#
	# 머리 위는 아무도 안 쓰는 자리다. 거기에 얇은 고리 하나면 색이 섞일 곳이
	# 없다. 발밑 링은 그대로 둔다 - 둘이 같은 말을 두 번 하는 것이 이 경우엔
	# 낭비가 아니다.
	if unit.team == Unit.TEAM_ENEMY and shown_alive():
		var hy := -R - 13.0
		var hr := 11.0
		# 바깥으로 번지는 붉은 잔광. 두 겹이면 발광처럼 읽힌다.
		draw_arc(Vector2(0, hy), hr + 2.5, 0, TAU, 20, Color(1.0, 0.18, 0.24, 0.16), 4.0)
		draw_arc(Vector2(0, hy), hr, 0, TAU, 20, Color(1.0, 0.26, 0.32, 0.85), 2.0)
		# 아래쪽을 살짝 눌러 타원으로 만든다. 머리 위에 **떠 있는** 고리로 읽힌다.
		draw_arc(Vector2(0, hy + 2.0), hr * 0.72, 0, TAU, 18,
			Color(1.0, 0.32, 0.38, 0.30), 1.0)
